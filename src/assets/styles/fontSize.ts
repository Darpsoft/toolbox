...
const calculateSize = (size: number): number => {
  const sizeFromPlatform = Platform.OS === "android" ? size + 2 : size;
  let sizeFont = 0;
  if (width >= 401 && width <= 420) {
    sizeFont = sizeFromPlatform * 1;
  } else if (width >= 381 && width <= 400) {
    sizeFont = sizeFromPlatform * 0.96;
  } else if (width >= 361 && width <= 380) {
    sizeFont = sizeFromPlatform * 0.92;
  } else if (width >= 341 && width <= 360) {
    sizeFont = sizeFromPlatform * 0.88;
  } else if (width >= 321 && width <= 340) {
    sizeFont = sizeFromPlatform * 0.83;
  } else if (width >= 301 && width <= 320) {
    sizeFont = Math.round(sizeFromPlatform * 0.8);
  }
